            ...
            loadSelectOptions(optionsData, clearExisting = true) {
                ...
                optionsData.forEach((option, index) => {
                    if (option && typeof option === 'object' && option.hasOwnProperty('value') && option.hasOwnProperty('text')) {
                        ...
                        // For simplicity with clearExisting=false, we might skip ID generation here if needed,
                        // or generate based on current list size + index. Let's generate based on current size.
            ...
            showComponent() {
                 if (this.$container && this.$container.length) {
                     ...
                     if (this.isDisabled) {
                         ...
                         if (this.options.enableSearch === true && this.$searchInput) this.$searchInput.prop('disabled', true).hide(); // Explicit boolean check
                     } else {
                         ...
                         // Only enable Select All if not single select (use boolean value)
                         if (this.options.singleSelect === false && this.$selectAllDiv && this.$selectAllDiv.length) this.$selectAllDiv.attr('aria-disabled', 'false').attr('tabindex', '0').show(); // Explicit boolean check
                         ...
                         // Enable search input if the option is set and show it (use boolean value)
                         if (this.options.enableSearch === true && this.$searchInput && this.$searchInput.length) this.$searchInput.prop('disabled', false).show(); // Explicit boolean check
                     }
